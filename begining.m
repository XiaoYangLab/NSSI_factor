classdef begining < matlab.apps.AppBase
    ...
    properties (Access = public)
        SurveyResults = NaN(1,29); % 改为用NaN初始化，区分未回答和选择0的情况         %重要！！当有新题目增加或删减时需要更改题目数量（例如共29题则改为SurveyResults = NaN(1,29);）
        QuestionsAnswered = false(1,29); % 新增：跟踪问题是否已回答        %重要！！当有新题目增加或删减时需要更改题目数量（例如共29题则改为 QuestionsAnswered = false(1,29);）
    end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            InstructionText.Text = ['你最初开始实施自伤（不是想自杀的伤害）的原因是什么？'  ...
    ...
    methods (Access = private)
        ...
        % % '30. “沉迷”于自伤行为';
            ...
            data = [questions, repmat({false}, 29, 5)];         %重要！！添加或删除问题时需要修改此列数，如：总共29个问题则改为data = [questions, repmat({false}, 29, 5)];
