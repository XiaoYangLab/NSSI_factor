classdef month < matlab.apps.AppBase
    ...
    properties (Access = public)
        SurveyResults = NaN(1,30); % Use NaN to initialize, distinguish unanswered from choosing 0
        QuestionsAnswered = false(1,30); % Track whether each question has been answered
    end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            InstructionText.Text = ['If you have self-injured in the past month (not intended as a suicide attempt), '  ...
                                     'what were your reasons? Below are some possible reasons — please check the ones that apply to you.'];
    ...
    methods (Access = private)
        ...
        '30. "Indulge" in self-harming behaviour';
                };
            ...
            app.SurveyResults = NaN(1,30); % Use NaN to indicate unanswered
            app.QuestionsAnswered = false(1,30); % Initialize answered status
        end
        ...
        function UITableCellEdit(app, event)
            ...
            % Only handle response columns (2–6)
            if indices(2) >= 2 && indices(2) <= 6
                row = indices(1);  % Question index
                col = indices(2);  % Option column

                % Set all options in current row to false (single selection)
                for i = 2:6
                ...
                % Update result matrix (column index - 2 = score 0–4)
        ...
        function SubmitButtonPushed(app, event)
            ...
            % Resume (can add result saving here)
            uiresume(app.UIFigure);
            % e.g., save('survey_results.mat', 'app.SurveyResults');
        end
    end
