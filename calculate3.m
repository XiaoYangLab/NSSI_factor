  ...
  if begin
    % 自动关闭App
        ...
        delete(question2);
            ...
            nssi = sum(newX1);
            nssimonth = sum(newX2);
            disp('最初nssi总分:'); disp(nssi);
            disp('一个月内nssi总分:'); disp(nssimonth);
            ...
            disp('最初社会功能总分:');disp(social);
            disp('最初自我功能总分:');disp(individual);
            ...
            disp('当前社会功能总分:');disp(socialmonth);
            disp('当前自我功能总分:');disp(individualmonth);
    ...
    %显示结果
    disp('w1到各中心点的距离:'); disp(dist_w1');
    disp('w1最近的center:'); disp(idx_w1);
    disp('最小距离:'); disp(min_dist_w1);

    disp('w2到各中心点的距离:'); disp(dist_w2');
    disp('w2最近的center:'); disp(idx_w2);
    disp('最小距离:'); disp(min_dist_w2);
    
    % 根据最近的center输出结果
    center_names = {'自我型', '社会型', '未分化型'};

    % fprintf('w1 属于 %s (距离: %.4f)\n', center_names{idx_w1}, min_dist_w1);
    % fprintf('w2 属于 %s (距离: %.4f)\n', center_names{idx_w2}, min_dist_w2);

window1 = msgbox(...
    sprintf('你最初的自伤原因有80%%的可能来源于%s。\n你近一个月的自伤原因有80%%的可能来源于%s。\n最初nssi总分%d,一个月内nssi总分:%d。\n最初社会功能总分:%d, 最初自我功能总分:%d。\n当前社会功能总分:%d, 当前自我功能总分:%d', center_names{idx_w1}, center_names{idx_w2},nssi, nssimonth,social, individual,socialmonth, individualmonth), ...
    '完成', ...
    'help' ... % 使用帮助图标
);
